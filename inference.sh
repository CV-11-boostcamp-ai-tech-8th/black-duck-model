...
# - YAML 설정 파일 배열을 받아 순차적으로 inference 실행
...
CONFIGS=(
    "configs/inference/idx1.yaml"
    "configs/inference/idx2.yaml"
    "configs/inference/idx3.yaml"
    "configs/inference/idx4.yaml"
)
...
# 각 설정 파일에 대해 inference 실행
for i in "${!CONFIGS[@]}"; do
    CONFIG="${CONFIGS[$i]}"
    INDEX=$((i + 1))
    
    echo "----------------------------------------------------------------------"
    echo "[$INDEX/${#CONFIGS[@]}] Inference 시작"
    echo "----------------------------------------------------------------------"
    echo "설정 파일: $CONFIG"
    ...
    # 로그 파일명 생성
    LOG_FILE="$LOG_DIR/inference_${VIDEO_NAME}_${VERSION}_${TIMESTAMP}.log"
    echo "로그 파일: $LOG_FILE"
    echo ""
    
    # Inference 실행 및 로그 저장 (tee로 실시간 출력 + 파일 저장)
    if $PYTHON_CMD "$INFERENCE_SCRIPT" "$CONFIG" 2>&1 | tee "$LOG_FILE"; then
        echo "✓ 성공"
