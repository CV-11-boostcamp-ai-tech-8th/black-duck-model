...
def save_event_package(event, video_path, fps, save_dir, clip_idx, save_crops=True):
    ...
    end_f = event["end_frame"] + int(fps * 2.0)
    ...
    if save_crops and temp_candidates:
        temp_candidates.sort(key=lambda x: x['score'], reverse=True)
        crop_dir = os.path.join(event_dir, "crops")
        os.makedirs(crop_dir, exist_ok=True)
        
        for i, cand in enumerate(temp_candidates[:60]):
            cv2.imwrite(os.path.join(crop_dir, f"crop_{i:02d}.jpg"), cand['img'])

    return {
        'clip_path': final_path,
        'vehicle_crops': len(temp_candidates[:60])
    }